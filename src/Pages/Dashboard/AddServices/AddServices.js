...
const AddServices = () => {
  ...
  console.log(getImg.current.files);
  ...
  const handelRegister = (e) => {
    ...
    const img = getImg.current.files;
    ...
    fetch(" https://mimoo-server.vercel.app/services", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        if (data.insertedId) {
          ...
          getPackName.current.value =
            getCountryName.current.value =
            getContinentName.current.value =
            getFollowers.current.value =
            getImg.current.value =
            getRating.current.value =
            getRating.current.value =
            getDescription1.current.value =
            getDescription2.current.value =
              "";
        }
      });
    e.preventDefault();
          ...
          <form onSubmit={handelRegister} className="register-form mt-6">
            ...
            <div>
              <input
                ...
                step="0.01"
                ref={getRating}
                placeholder="Package Travel Rating"
              />
            </div>
            <br />
            <div>
